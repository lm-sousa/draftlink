  ...
  const { results } = await db
    .prepare(
      `SELECT id, title, project, status, is_public, created_at, updated_at FROM drafts
       WHERE owner_user_id = ?1 AND (?2 = '%%' OR title LIKE ?2 ESCAPE '\\' OR project LIKE ?2 ESCAPE '\\' OR body LIKE ?2 ESCAPE '\\') AND ((?3 = 'archived') = (status = 'archived'))
       AND (?4 = '' OR project = ?4)
  ...
  const { results } = await db
    .prepare(
      `SELECT d.id, d.title, d.project, d.status, d.is_public, d.created_at, d.updated_at, u.login AS owner_login
       ...
       WHERE a.github_id = ?1 AND (?2 = '%%' OR d.title LIKE ?2 ESCAPE '\\' OR d.project LIKE ?2 ESCAPE '\\' OR d.body LIKE ?2 ESCAPE '\\') AND ((?3 = 'archived') = (d.status = 'archived'))
